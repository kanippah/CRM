...
- **User Management:** Secure user invitation system where users set their own passwords, and admin never has direct access to them. Features include user deactivation/activation without deletion. Admin can delete pending invitations that haven't been accepted. Admin can assign phone numbers (DIDs) to users for Twilio-based outbound calling.
- **Contact Management:** Includes duplicate detection based on normalized phone numbers and case-insensitive company names. Supports contact types (Individual/Company). Admin view includes "Assigned To" column showing contact ownership, reassignment capability, and clickable contact names for detailed view (includes call history and projects). Industry field allows categorization with custom industry management. All contacts automatically assigned to creating user. Sales users can return contacts to leads pool for re-nurturing. Click-to-call buttons integrated in contact phone numbers for Twilio-based calling.
- **Call Tracking:** Allows logging calls with outcomes, duration, and associated contacts. Features call updates for chronological note-taking. Latest update displayed in notes column with 50-character truncation and tooltip for full text. Sales users can add updates and edit calls but cannot delete (admin-only). Clickable contact names to view full call history. Twilio integration enables click-to-call functionality with automatic call logging, recording, and status tracking.
- **Twilio Integration:** Click-to-call functionality using Twilio API for browser-based outbound calling. Each user can be assigned a DID (phone number) by admin for caller ID. Calls are automatically logged with Twilio call SID, status updates via webhooks, call duration tracking, and recording URL storage. Status callbacks update call records in real-time as calls progress (initiated, ringing, answered, completed).
...
- **Database Schema:** Utilizes PostgreSQL with specific tables for Users (with phone_number field), Password Resets, Invitations, Contacts, Calls (with twilio_call_sid, twilio_status, twilio_recording_url fields), Call Updates, Projects, Settings, Leads, Industries, and Interactions. Tables include necessary indexes for performance. Auto-migration system ensures schema updates on app startup.
- **API Endpoints:** A comprehensive set of API endpoints for all CRUD operations and specific functionalities like authentication, session management, lead conversion, project stage updates, contact reassignment, invitation management, industry management, and Twilio calling (twilio.call, twilio.status).
...
- **Twilio:** Integrated via Replit connectors for browser-based outbound calling. Uses environment variables (`TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`) for API authentication. Provides click-to-call functionality, automatic call logging, call recording, and real-time status updates via webhooks.