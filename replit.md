...
- **User Management:** Passwordless invitation system where admin invites users by email and role only. Invited users receive a magic link (valid 24 hours) and only need to provide their full name to complete registration - no password required. Features include user deactivation/activation without deletion. Admin can delete pending invitations that haven't been accepted.
